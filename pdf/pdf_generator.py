...
class PDFGenerator:
    ...
    def generate_pdf(self, article_data: list[dict], file_path: str) -> None:
        ...
        for article in article_data:
            ...
            # 일본어 문장 추가 (줄바꿈 적용)
            y_position = self.multi_line_text(c, f"일본어: {original_sentence}", x_margin, y_position, max_width, 12)

            # 번역된 한국어 문장 추가 (줄바꿈 적용)
            y_position = self.multi_line_text(c, f"한국어: {translated_sentence}", x_margin, y_position, max_width, 12)
            ...
            for kanji in kanji_info:
                ...
                text = f"- {kanji_text}({kana_text}): {meaning}"
                y_position = self.multi_line_text(c, text, x_margin, y_position, max_width, 12)
    ...
    def multi_line_text(self, canvas_obj, text, x_margin, y_position, max_width, font_size):
        """
        ...
        for word in text:
            if canvas_obj.stringWidth(line + word, "NotoSans", font_size) <= max_width:
                ...
                if y_position < 20 * mm:
                    canvas_obj.showPage()
                    canvas_obj.setFont("NotoSans", 12)
